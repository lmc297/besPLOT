...
server <- function(input, output) {
  ...
  output$metadata <- renderTable({
    ...
    if (is.null(input$file1)){
      ...
      vmat<-read.table(finalfiles, header=T, sep=finalsep)
  ...
  output$listPC1 <- renderUI({
    ...
    if (is.null(infile)){
      ...
      vmat<-read.table(infile, header = T, sep=matrix.delim)
      rownames(vmat)<-vmat[,1]
      vmat<-vmat[,2:ncol(vmat)]
      if(vselect=='1'){
  ...
  output$virbarchart <- renderPlot({
    ...
    myplot<-virulence(task="makeplot")
    print(myplot)
    myplot
    #return(myplot)
  })
